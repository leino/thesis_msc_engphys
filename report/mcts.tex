...
It is assumed that we have a function, \texttt{value}, which gives a real number intended to represent the value of a given node for First, but which we can apply only to terminal positions.
...
Apart from that, we will also assume that we have a function named \texttt{choices}, which we can apply to a node in order to find the set of possible choices of nodes that the player in turn could move to.

We further assume that our nodes can carry ``MCTS data'' of the following format.
\begin{lstlisting}[frame=single]
data MCTSNodeData = MCTSNodeData { visitCount :: Int,
                                   score :: Score }
\end{lstlisting}
The \texttt{Score} type is just a synonym for a real number type, like \texttt{Float}. We can view a node's MCTS data, if it has been explored, by means of the \texttt{getMCTSData} function, and we can update it by means of \texttt{setMCTSData}.
...
Note that $\texttt{s}$ represents the score from the point of view of the opponent of the player in turn, since \texttt{recon} was applied to a child of \texttt{position}.

The \texttt{explore} function is the core of the algorithm. The return value of the function is a tuple of a score together with the result of exploring the node. Just like \texttt{recon}, the score it returns is relative to the player who's turn it is in the given node.
...
If the explored node is not terminal, then we select the best child as described in \ref{blah}, explore that child recursively with another call to \texttt{explore} and use the result to return an updated node.

Now we can easily write the final MCTS strategy. It needs two parameters: a number of iterations and a node to work on.
\begin{lstlisting}[frame=single]
...
mctsStrategy numSteps node = do
  ...
  cExp = 1.0 / (sqrt 2.0)
\end{lstlisting}
The base case of zero iterations is listed first. It uses the function \texttt{findBestMove} which in turn uses the comparison function \ref{code:blah} to determine the best move for the player in turn, according to the currently available MCTS data. In the other case, one round of \texttt{explore} is executed, and the strategy is applied recursively to the resulting node.

One important ingredient we'll leave out is the \texttt{compareChildren} function. It takes a node and two children of the node, and returns an ordering.
It allows us to sort a set of children, and therefore to write \texttt{popBestChild} and \texttt{findBestMove} with relative ease.
The function is just a straight encoding of the rules mentioned in \ref{blah}, and so is omitted here.
