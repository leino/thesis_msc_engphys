...
if len(sys.argv) >= 2:
    ...
    args_hypergraphs = ["time", "runhaskell", "addhypergraphs.hs", database_filename, str(num_vertices), str(num_edges), '-d1:%(min_edge_size)d' % {'min_edge_size': min_edge_size}, '-D%(num_edges)d:%(max_edge_size)d' % {'num_edges': num_edges, 'max_edge_size': max_edge_size}]
